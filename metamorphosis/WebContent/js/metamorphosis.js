...
page.register = (type, engine) => page.engines[type] = engine;

page.register("text/x-handlebars-template", (info) => {
...
page.register("text/x-dust-template", (info) => {
...
page.render = (element, data, ...options) => {
  ...
  const engine = page.engines[script.attr("type")];
  engine({
    source: script.html(),
    data: data,
...
page.pdf = (url,callback) => {
	head.load("js/pdfmake.min.js", "js/vfs_fonts.js", () => {
		app.get(url, (data) => {callback(data);});
	});
};

var print = (entity) => pdfMake.createPdf(doc(entity)).print();

var pdf = (entity) => pdfMake.createPdf(doc(entity)).open();
